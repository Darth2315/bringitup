...
window.addEventListener('DOMContentLoaded', function () {
  ...
  new _modules_playVideo__WEBPACK_IMPORTED_MODULE_2__["default"]('.module__video-item .play', '.overlay').init(); // Альтернативный метод вызова класса
    ...
    value: function initMask() {
      ...
      function createMask(event) {
        ...
        if (event.type === 'blur') {
          ...
          setCursorPosition(this.value.length, this); // console.log(this.value.length);
        }
      }
  ...
  _createClass(ShowInfo, [{
    ...
    value: function init() {
      this.btns.forEach(function (item) {
        item.addEventListener('click', function () {
          try {
            var contentBlock = item.closest('.module__info-show').nextElementSibling; // contentBlock.style.display = 'block';
